...
try:
    import _modern_colorthief_gpu as gpu
except ImportError:
    gpu = None
...
@pytest.mark.skipif(gpu is None, reason="GPU module not available")
class TestGpuPalette:
    """Tests for GPU palette extraction."""

    def test_palette_not_empty(self, solid_red_pixels):
        palette = gpu._get_palette_gpu(solid_red_pixels, 100, 100)
    ...
    def test_solid_green_dominant(self, solid_green_pixels):
        palette = gpu._get_palette_gpu(solid_green_pixels, 100, 100)
        r, g, b = palette[0]
        assert r < 55
        assert g > 200
        assert b < 55
    ...
    def test_solid_blue_dominant(self, solid_blue_pixels):
        palette = gpu._get_palette_gpu(solid_blue_pixels, 100, 100)
        r, g, b = palette[0]
        assert r < 55
        assert g < 55
        assert b > 200
    ...
    def test_color_count_bound(self, solid_red_pixels):
        palette = gpu._get_palette_gpu(solid_red_pixels, 100, 100, color_count=5)
        assert len(palette) <= 5

    def test_no_duplicate_colors(self, solid_red_pixels):
        palette = gpu._get_palette_gpu(solid_red_pixels, 100, 100, color_count=10)
        assert len(palette) == len(set(palette))

    def test_deterministic(self, solid_red_pixels):
        p1 = gpu._get_palette_gpu(solid_red_pixels, 100, 100)
        p2 = gpu._get_palette_gpu(solid_red_pixels, 100, 100)
        assert p1 == p2

    def test_quality_parameter(self, solid_red_pixels):
        for q in (1, 5, 10):
            palette = gpu._get_palette_gpu(solid_red_pixels, 100, 100, quality=q)
            assert len(palette) > 0

    def test_error_on_empty_pixels(self):
        with pytest.raises(ValueError):
            gpu._get_palette_gpu(b"", 0, 0)

    def test_error_on_zero_width(self, solid_red_pixels):
        with pytest.raises(ValueError):
            gpu._get_palette_gpu(solid_red_pixels, 0, 100)

    def test_error_on_zero_height(self, solid_red_pixels):
        with pytest.raises(ValueError):
            gpu._get_palette_gpu(solid_red_pixels, 100, 0)

    def test_error_on_mismatched_size(self):
        """10x10 RGBA = 400 bytes, but only 100 provided."""
        with pytest.raises(ValueError):
            gpu._get_palette_gpu(bytes(100), 10, 10)
...
class TestGpuColor:
    ...
    def test_color_is_tuple_of_3(self, solid_red_pixels):
        color = gpu._get_color_gpu(solid_red_pixels, 100, 100)
        assert len(color) == 3

    def test_color_valid_rgb(self, solid_red_pixels):
        r, g, b = gpu._get_color_gpu(solid_red_pixels, 100, 100)
        assert 0 <= r <= 255
        assert 0 <= g <= 255
        assert 0 <= b <= 255
    ...
    def test_deterministic(self, solid_red_pixels):
        c1 = gpu._get_color_gpu(solid_red_pixels, 100, 100)
        c2 = gpu._get_color_gpu(solid_red_pixels, 100, 100)
        assert c1 == c2

    def test_error_on_empty_pixels(self):
        with pytest.raises(ValueError):
            gpu._get_color_gpu(b"", 0, 0)
