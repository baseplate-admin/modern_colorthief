...
// ext-php-rs exposes gpu_get_palette() and gpu_get_color() as global functions
...
test('gpu get_color returns dominant color', function () {
    $color = gpu_get_color($this->redPixels, 1, 1, 1);
    expect($color)->toBe([255, 0, 0]);
});

test('gpu get_color returns valid RGB values', function () {
    $color = gpu_get_color($this->greenPixels, 3, 3, 1);
...
test('gpu error on empty pixels', function () {
    expect(fn () => gpu_get_palette('', 1, 1, 5, 1))->toThrow();
    expect(fn () => gpu_get_color('', 1, 1, 1))->toThrow();
});

test('gpu error on mismatched pixel data', function () {
    // 4 bytes but claiming 2x2 (needs 16 bytes for RGBA)
    expect(fn () => gpu_get_palette("\xff\x00\x00\xff", 2, 2, 5, 1))->toThrow();
});

test('gpu deterministic results for same input', function () {
    $result1 = gpu_get_palette($this->greenPixels, 3, 3, 5, 1);
    $result2 = gpu_get_palette($this->greenPixels, 3, 3, 5, 1);
    expect($result1)->toEqual($result2);
});

test('gpu deterministic get_color results', function () {
    $color1 = gpu_get_color($this->redPixels, 1, 1, 1);
    $color2 = gpu_get_color($this->redPixels, 1, 1, 1);
    expect($color1)->toEqual($color2);
});

test('gpu different images produce different dominant colors', function () {
    $redColor = gpu_get_color($this->redPixels, 1, 1, 1);
    $greenColor = gpu_get_color($this->greenPixels, 3, 3, 1);
    expect($redColor)->not->toEqual($greenColor);
});

test('gpu quality parameter is accepted', function () {
    $color1 = gpu_get_color($this->greenPixels, 3, 3, 1);
    $color10 = gpu_get_color($this->greenPixels, 3, 3, 10);
