...
beforeAll(async () => {
    try {
        const mod = await import('../pkg/modern_colorthief_node_wasm.js');
...
describe('Solid color', () => {
    it('detects pure red', async () => {
        ...
        const palette = await getPalette(pixels, width, height, 5, 10);
...
describe('Palette properties', () => {
    ...
    it('palette length does not exceed color_count', async () => {
        ...
        for (const count of [3, 5]) {
            const palette = await getPalette(pixels, width, height, count, 10);
    ...
    it('returns no duplicate colors', async () => {
        await ready();
        const { pixels, width, height } = solidPixels(100, 150, 200);
        const palette = await getPalette(pixels, width, height, 20, 10);
...
describe('Determinism', () => {
    it('same palette for same pixels', async () => {
        ...
        const p1 = await getPalette(pixels, width, height, 5, 10);
        const p2 = await getPalette(pixels, width, height, 5, 10);
    ...
    it('same color for same pixels', async () => {
        await ready();
        const { pixels, width, height } = solidPixels(180, 90, 45);
        const c1 = await getColor(pixels, width, height, 10);
        const c2 = await getColor(pixels, width, height, 10);
    ...
    it('deterministic on multi-color image', async () => {
        ...
        const p1 = await getPalette(pixels, width, height, 10, 5);
        const p2 = await getPalette(pixels, width, height, 10, 5);
        expect(p1).toEqual(p2);
    });
});
...
describe('Quality bounds', () => {
    ...
    it('quality=5 is valid', async () => {
        await ready();
        const { pixels, width, height } = solidPixels(200, 100, 50);
        const palette = await getPalette(pixels, width, height, 5, 5);
        expect(palette.length).toBeGreaterThan(0);
    });
});
...
describe('Deduplication', () => {
    it('no duplicates with high color_count', async () => {
        await ready();
        const { pixels, width, height } = solidPixels(100, 150, 200);
        const palette = await getPalette(pixels, width, height, 50, 10);
        const keys = palette.map(c => `${c[0]},${c[1]},${c[2]}`);
        expect(new Set(keys).size).toBe(keys.length);
...
describe('Edge cases', () => {
    ...
    it('higher color_count returns more colors', async () => {
        ...
        const small = await getPalette(pixels, width, height, 3, 10);
        const large = await getPalette(pixels, width, height, 10, 10);
...
describe('Error handling', () => {
    it('rejects empty pixels — palette', async () => {
        await ready();
        await expect(getPalette(new Uint8Array(0), 0, 0, 5, 10)).rejects.toThrow();
    });

    it('rejects insufficient pixel data — palette', async () => {
        await ready();
        await expect(getPalette(new Uint8Array([255, 0, 0]), 1, 1, 5, 10)).rejects.toThrow();
    });

    it('rejects empty pixels — color', async () => {
        await ready();
        await expect(getColor(new Uint8Array(0), 0, 0, 10)).rejects.toThrow();
    });

    it('rejects insufficient pixel data — color', async () => {
        await ready();
        await expect(getColor(new Uint8Array([255, 0, 0]), 1, 1, 10)).rejects.toThrow();
    });
});
...
describe('Concurrency', () => {
    it('handles concurrent palette calls', async () => {
        ...
        const results = await Promise.all(
            Array.from({ length: 5 }, () => getPalette(pixels, width, height, 10, 10))
        );
    ...
    it('handles concurrent color calls', async () => {
        ...
        const { pixels, width, height } = solidPixels(100, 150, 200);
        const results = await Promise.all(
            Array.from({ length: 5 }, () => getColor(pixels, width, height, 10))
        );
        expect(results.length).toBe(5);
    ...
    it('concurrent calls produce consistent results', async () => {
        ...
        const { pixels, width, height } = solidPixels(180, 90, 45);
        const results = await Promise.all(
            Array.from({ length: 4 }, () => getPalette(pixels, width, height, 10, 10))
        );
