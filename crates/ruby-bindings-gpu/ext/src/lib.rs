...
mod tests {
    ...
    fn test_palette_solid_red() {
        ...
        assert!(palette.iter().any(|c| c[0] > 200 && c[1] < 55 && c[2] < 55));
    }
    ...
    fn test_quality_zero() {
        let pixels = solid_pixels(170, 85, 220, 100);
        let rs = rstring(&pixels);
        let palette = get_palette(rs, 10, 10, 5, 0).unwrap();
        assert!(!palette.is_empty(), "quality=0 should be clamped to 1");
    }
    ...
    fn test_quality_high() {
        let pixels = solid_pixels(170, 85, 220, 100);
        let rs = rstring(&pixels);
        let palette = get_palette(rs, 10, 10, 5, 100).unwrap();
        assert!(!palette.is_empty());
    }

    #[ruby_test]
    fn test_color_quality_zero() {
        let pixels = solid_pixels(100, 150, 200, 100);
        let rs = rstring(&pixels);
        let color = get_color(rs, 10, 10, 0).unwrap();
        assert_eq!(color.len(), 3);
    }

    #[ruby_test]
    fn test_color_quality_high() {
        let pixels = solid_pixels(100, 150, 200, 100);
        let rs = rstring(&pixels);
        let color = get_color(rs, 10, 10, 100).unwrap();
        assert_eq!(color.len(), 3);
    }
