...
fn init_colorthief_gpu_ruby() -> Result<(), Error> {
    ...
    mod_colorthief_gpu.define_singleton_method("get_palette", ruby_function!(get_palette, 5))?;
    mod_colorthief_gpu.define_singleton_method("get_color", ruby_function!(get_color, 4))?;
    Ok(())
}
